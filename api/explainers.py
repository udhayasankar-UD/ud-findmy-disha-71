...
def generate_why_tags(internship: Dict[str, Any], user_profile: Dict[str, Any]) -> List[str]:
    ...
    if matched_skills:
        # Ensure user_profile.get('skills',[]) is not empty to avoid division by zero.
        # This part of the explanation does not directly cause the crash,
        # but the len calculation might be problematic if user_profile['skills'] is empty
        # after filtering. Added a check in get_skill_match_explanation.
        num_user_skills = len(user_profile.get('skills', []))
        if num_user_skills == 0:
            skill_match_info = "No skills specified in user profile."
        else:
            skill_details = [f"{m['skill']} ({m['mode']})" for m in matched_skills]
            skill_match_info = f"Matches {len(matched_skills)}/{num_user_skills} skills: {', '.join(skill_details)}"
        tags.append(skill_match_info)
...
def generate_detailed_explanation(internship: Dict[str, Any], user_profile: Dict[str, Any], weights: Dict[str, float]) -> Dict[str, Any]:
    ...
    if num_user_skills == 0:
        ...
        skill_score = len(internship.get('skill_matches', [])) / num_user_skills * 100
    
    location_score = internship.get('location_score', 0.0) * 100
    
    # --- FIX START --- (These were already corrected in the previous round)
    # These scores are already on a 0-100 scale from rules.py, so no multiplication is needed.
    # The default value is also changed from 0.5 to 50 to match the 0-100 scale.
    stipend_score = internship.get('stipend_score', 50.0)
    date_score = internship.get('date_score', 50.0)
    # --- FIX END ---
...
def get_skill_match_explanation(matched_skills: List[Dict[str, str]], user_skills: List[str]) -> str:
    # Avoid division by zero if user has no skills listed
    if not user_skills:
        return "No skills in user profile to match against."
...
def get_stipend_explanation(job_stipend: Optional[float], min_stipend: Optional[float]) -> str:
    if pd.isna(job_stipend) or min_stipend is None:
        return "Stipend information incomplete"
    
    if job_stipend >= min_stipend:
        # Avoid division by zero if min_stipend is 0
        if min_stipend > 0:
        ...
        else: # Handle case where min_stipend is 0
             return f"Meets your minimum stipend requirement: ₹{int(job_stipend):,}"
    else:
        # Avoid division by zero if min_stipend is 0
        if min_stipend > 0:
        ...
        else: # Handle case where job_stipend is somehow negative
            return f"Stipend is ₹{int(job_stipend):,}"
