...
def parse_stipend(stipend_str: str) -> Optional[float]:
    """Parses a stipend string, handling commas, ranges, and periods."""
    ...
    # --- FIX START ---
    # Remove commas from the string to handle numbers like "15,000"
    stipend_str = stipend_str.replace(',', '')
    # --- FIX END ---
...
def parse_date(date_str: str) -> Optional[str]:
    try:
        # Handles various date formats including 'DD-MM-YYYY'
        return parse(date_str, dayfirst=False, yearfirst=False).strftime('%Y-%m-%d')
...
def score_stipend(job_stipend: Optional[float], min_stipend: Optional[float]) -> float:
    if job_stipend is None or min_stipend is None or min_stipend <= 0:
        return 50.0 # Return a neutral score
    
    if job_stipend >= min_stipend:
        return 100.0
    
    # Calculate the ratio and scale it to 0-100
...
def score_deadline(apply_by_date: Optional[str], available_from: Optional[str]) -> float:
    ...
    try:
        ...
        if days_until > 30:  # More than a month
            ...
            return 90.0
        else:  # Less than a week (but not passed)
            return 70.0

    except (ValueError, TypeError):
        return 50.0  # Fallback for parsing errors