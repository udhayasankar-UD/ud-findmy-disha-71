...
def create_job_embeddings(df: pd.DataFrame, model: SentenceTransformer, cache_path: str, recompute: bool = False) -> torch.Tensor:
    ...
    # CORRECTED: Use 'title' and 'skills' columns from the new CSV format
    # Ensure these columns exist, provide a fallback if they don't, although they should now.
    df['combined_text'] = df.get('title', df.get('role', '')).astype(str) + " " + \
                          df.get('skills', df.get('Skills', '')).fillna('').astype(str)
...
def create_skill_embeddings(df: pd.DataFrame, model: SentenceTransformer, cache_path: str, recompute: bool = False) -> Dict[str, torch.Tensor]:
    ...
    # Ensure 'parsed_skills' column is present before applying
    if 'parsed_skills' in df.columns:
